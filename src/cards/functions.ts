...
export async function deleteCardsByTheme(id:number): Promise<void> {
  if (!cards[id]) {
...
export async function updateTheme(id:number, newName:string):Promise<void> {
  themes[id] = newName;
}
export async function getCards(): Promise<{}[]> {
  return Promise.resolve(cards);
}
export async function deleteCardFromThemeByID(theme:number, id:number) {
  if (!cards[theme][id]) {
...
export async function updateCardFromThemeByID(theme:number, id:number, word: string, translation:string) {
  if (!cards[theme][id]) {
    return Promise.reject(new Error('Card not found'));
  }
